...
const
  String1: PWideChar = #20'Terminating by handle 0x';
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        hProcess := ProcessHandles[i];
        ns := ZwTerminateProcess(hProcess, 0);

        if (ns = STATUS_SUCCESS) then
          OutputDebugStringW('Successfully terminated')
        else
          OutputDebugStringW('Holy fuck!!! Well you probably know whats next :)');
