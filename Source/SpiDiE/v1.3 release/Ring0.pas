...
begin
  ...
  RtlInitUnicodeString(@s1, '\Device');
  InitializeObjectAttributes(@Attr, @s1, OBJ_CASE_INSENSITIVE or
    OBJ_KERNEL_HANDLE, 0, nil);

  tmp := 0;
  ZwOpenDirectoryObject(@tmp, DIRECTORY_QUERY, @Attr);


  RtlInitUnicodeString(@PhysMemString, 'PhysicalMemory');

  InitializeObjectAttributes(@Attr, @PhysMemString, OBJ_CASE_INSENSITIVE or
    OBJ_KERNEL_HANDLE, tmp, nil);
